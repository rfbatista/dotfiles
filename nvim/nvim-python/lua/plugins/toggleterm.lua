return {
  ...
  config = function()
    ...
    vim.api.nvim_set_keymap(
      "n",
      "<A-9>",
      "<cmd>ToggleTerm 2 size=20 direction=float name=Terminal<CR>",
      { noremap = true, silent = true }
    )
    vim.api.nvim_set_keymap(
      "n",
      "<A-8>",
      "<cmd>ToggleTerm 3 size=20 direction=float name=Terminal<CR>",
      { noremap = true, silent = true }
    )
    ...
    function _START_NGROK()
      vim.ui.input({ prompt = "Para qual porta mapear o ngrok?" }, function(str)
        local command = string.format('3TermExec cmd="ngrok http --host-header=rewrite http://localhost:%s"', str)
        vim.cmd(command)
      end)
    end
